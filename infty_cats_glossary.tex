...
\begin{longtable}{ |p{3.2cm}||p{7cm}|p{7cm}|p{8cm}|  }
 ...
 \hline\hline
 Accessible category & \(\mathcal{C}\) is locally small, admits \(\kappa\)-filtered colimits, and there is a set of \(\kappa\)-compact objects which generates \(\mathcal{C}\) under \(\kappa\)-filtered colimits. (\autocite{accessible}, Def 2.1) & \(\mathcal{C}\) is locally small, admits \(\kappa\)-filtered colimits, and there is an essentially small subcategory of \(\kappa\)-compact objects which generates \(\mathcal{C}\) under \(\kappa\)-filtered colimits\footnote{I.e. for any full subcategory \(\mathcal{C}'\subq \mathcal{C}\) containing every element of \(\mathcal{C}^\kappa\), if \(\mathcal{C}'\) is stable under small \(\kappa\)-filtered colimits, then \(\mathcal{C}'=\mathcal{C}\). (\autocite{htt}, p328)}. (\autocite{infty_accessible}, Def 2.1)\footnote{\textcolour{red}{(Should reconcile with \autocite{htt}, 5.4.3).}} & -\\
 \hline
...
\(\bullet\) For every finite collection of objects \(C_1, ..., C_n \in \mathcal{O}^\otimes_{\ang{1}}\), there exists an object \(C \in \mathcal{O}^\otimes_{\ang{n}}\) and a collection of \(p\)-cocartesian morphisms \(C\to C_i\) covering \(\rho^i : \ang{n}\to \ang{1}\).
 & \textcolour{red}{[todo]}\\
\hline
(Coloured) operad map & \textcolour{red}{[todo]} & For \(\infty\)-operads \(p : \mathcal{O}^\otimes \to N(\textbf{Fin}_*)\), \(q : \mathcal{O}'^\otimes \to N(\textbf{Fin}_*)\), an \(\infty\)-operad map \(\mathcal{O}^\otimes \to \mathcal{O}'^\otimes\) is a map of simplicial sets \(f : \mathcal{O}^\otimes \to \mathcal{O}'^\otimes\) such that the diagram below commutes,
\[\begin{tikzcd}
\mathcal{O}^\otimes \arrow[rr, ""] \arrow[dr, swap, ""]  & & \mathcal{O}'^\otimes \arrow[dl, ""] \\ 
  & N(\textbf{Fin}_*) & 
 \end{tikzcd}\] 
and the functor \(f\) carries inert morphisms\footnote{I.e. \(p\)-cocartesian morphisms \(g\) with \(p(g) : \ang{m}\to \ang{n}\) such that for each \(i \in \ang{n}^\circ\), the inverse image \(g^{-1}\{i\}\) has exactly one element (\autocite{ha} Def 2.1.1.8 and 2.1.2.3).} in \(\mathcal{O}^\otimes\) to inert morphisms in \(\mathcal{O}'^\otimes\).  & \textcolour{red}{[todo]} \\
\hline
...
\begin{longtable}{ |p{5cm}||p{10cm}|}
 ...
 Name & Definition\\
  \hline\hline
\(\textbf{Alg}_\mathcal{C}(\mathcal{D})\) (\(p : \mathcal{C}^\otimes \to N(\textbf{Fin}_*)\), \(q : \mathcal{D}^\otimes \to N(\textbf{Fin}_*)\) \(\infty\)-operads) & The full subcategory of \(\textbf{Fun}_{N(\textbf{Fin}_*)}(\mathcal{C}^\otimes, \mathcal{D}^\otimes)\)\footnote{This notation is defined nowhere I can see, but must just mean maps \(\mathcal{C}^\otimes \to \mathcal{D}^\otimes\) making the obvious triangle commute (\textcolour{red}{but is this commuting on the nose, or only up to homotopy?}).} spanned by the \(\infty\)-operad maps. (\autocite{ha}, Def 2.1.2.7) \\
\hline
\textbf{Assoc} (the associative operad) & The coloured operad with a single object \(\mathfrak{a}\), and for every finite set \(I\), \(\Hom(\{\mathfrak{a}\}_i, \mathfrak{a})\) is the set of linear orderings on \(I\). Given a map of finite sets \(\alpha : I \to J\) together with operations \(\phi_j\in \Hom(\{\mathfrak{a}\}_{\alpha(i)=j}, \mathfrak{a})\) and \(\psi \in \Hom(\{\mathfrak{a}_j, \mathfrak{a}\})\), we identify each \(\phi_j\) with a linear ordering \(\leq_j\) on the set \(\alpha^{-1}\{j\}\) and \(\psi\) with a linear ordering \(\leq'\) on the set \(J\). The composition of \(\psi\) with \(\{\phi_j\}\) corresponds to the linear ordering \(\leq\) on the set \(I\) which is defined by: \(i\leq i'\) if either \(\alpha(i) <_j \alpha(i')\) or \(\alpha(i)=j=\alpha(i')\) and \(i \leq_j i'\). % so now it should be clear
(\autocite{ha}, Def 4.1.1.1)\\
\hline
\(\text{Assoc}^\otimes\) (the associative \(\infty\)-operad) & \(N(\textbf{Assoc}^\otimes)\). (\autocite{ha}, Def 4.1.1.3) \\
\hline
\(\textbf{Assoc}^\otimes\) & The category whose objects are the objects of \(\textbf{Fin}_*\), and a morphism \(m \to n\) is given by a map \(\alpha : \ang{m}\to \ang{n}\) in \(\textbf{Fin}_*\) together with a collection of linear orderings \(\leq_j\) on \(\alpha^{-1}\{j\}\), for \(1 \leq j \leq n\). Composition of morphisms is determined by the composition laws on \(\textbf{Fin}_*\) and on \textbf{Assoc}. \autocite{ha}, Def 4.1.1.3\\
\hline 
\(\textbf{Fin}_*\) & The category whose objects are the sets \(\ang{n}=\{*, 1, 2, ..., n\}\), and a morphism \(\ang{m}\to \ang{n}\) is a map \(\alpha : \ang{m}\to \ang{n}\) such that \(\alpha(*)=*\).\\
\hline
...
\hline
  \(\mathcal{S}\) (the \(\infty\)-category of spaces) & The simplicial\footnotemark nerve \(N(\textbf{Kan})\). (\autocite{htt}, Def 1.2.16.1)\\
  \hline 
\(\widehat{\mathcal{S}}\) & The simplicial nerve \(N(\textbf{KAN})\). (\autocite{htt}, Rem 5.1.6.1)\\
\hline
\end{longtable}
